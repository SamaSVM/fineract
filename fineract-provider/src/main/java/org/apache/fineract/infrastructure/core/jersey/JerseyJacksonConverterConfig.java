...
public class JerseyJacksonConverterConfig {
    ...
    public ObjectMapper objectMapper(List<ValueSerializer<?>> serializers, List<ValueDeserializer<?>> deserializers,
        ...
        mergedSerializers.forEach(s -> customModule.addSerializer(s));
        for (ValueDeserializer<?> d : mergedDeserializers) {
            @SuppressWarnings("unchecked")
            Class<Object> type = (Class<Object>) d.handledType();
            customModule.addDeserializer(type, (ValueDeserializer<Object>) d);
        }
        ...
        return JsonMapper.builder()
                ...
                .addModule(customModule).addModule(new JacksonLocalDateArrayModule()).build();
    }
}
